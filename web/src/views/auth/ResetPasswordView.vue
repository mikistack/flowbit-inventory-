...
const submit = async () => {
  ...
  try {
    ...
    state.message = 'Password reset successfully. Redirecting to login...';
    setTimeout(() => {
  ...
  <div class="min-h-screen flex items-center justify-center bg-slate-100 px-4">
    <div class="bg-white rounded-3xl shadow-xl max-w-md w-full p-8 space-y-6">
      <div class="text-center">
        ...
        <h1 class="text-2xl font-semibold mt-2">Reset Password</h1>
        <p class="text-slate-500">Enter your new password to complete the reset.</p>
      </div>
      <form class="space-y-4" @submit.prevent="submit">
        <div>
          <label class="text-sm font-medium text-slate-600">Reset Token</label>
          <input
            v-model="form.token"
            type="text"
            required
